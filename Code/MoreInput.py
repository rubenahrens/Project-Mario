...
class EA():
    ...
    def process_state(self, oldObs, obs, output, info):
        oldObs = cv2.cvtColor(oldObs, cv2.COLOR_BGR2GRAY)
        oldObs = cv2.resize(oldObs, (self.width, self.length))
        oldObs = oldObs.flatten()
        obs = cv2.cvtColor(obs, cv2.COLOR_BGR2GRAY)
        obs = cv2.resize(obs, (self.width, self.length))
        obs = obs.flatten()

        state = np.append(obs, oldObs)
        state = np.append(state, output)
    ...
    def cal_fitness(self, para):
        ...
        output = 1
        # oldState = self.process_state(obs, info)
        while not done:
            state = self.process_state(oldObs, obs, output, info)
            oldObs = obs
    ...
    def main(self, config_file='newconfig'):
